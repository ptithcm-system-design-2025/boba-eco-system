...
export class ManagerService {
	...
	private handleCreateError(error: unknown, email: string): never {
		if (error instanceof Prisma.PrismaClientKnownRequestError) {
			switch (error.code) {
				case 'P2002': {
					const fieldDescription = this.getUniqueConstraintField(
						error,
						email
					);
					throw new ConflictException(
	...
	private async getManagerRoleId(): Promise<number> {
		...
		if (!managerRole) {
			throw new BadRequestException(
				'MANAGER role not found in the system.'
			);
		}
	 ...
	 * @throws {BadRequestException} For other database-related errors.
	 */
	private handleUpdateError(error: unknown, manager_id: number): never {
		if (error instanceof Prisma.PrismaClientKnownRequestError) {
			switch (error.code) {
				case 'P2025':
					throw new NotFoundException(
	 ...
	 * @throws {BadRequestException} For other database-related errors.
	 */
	private handleDeleteError(error: unknown, manager_id: number): never {

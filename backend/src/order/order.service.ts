...
export class OrderService {
				...
				if (products.length > 0) {
					...
					for (const productDto of products) {
						const productPriceInfo =
							await tx.product_price.findUnique({
								where: {
									product_price_id:
										productDto.product_price_id,
								},
							});
						if (!productPriceInfo)
							throw new NotFoundException(
						...
						newTotalAmount = newTotalAmount.plus(
							new Decimal(productPriceInfo.price).times(
								productDto.quantity
							)
						);
			...
			if (discounts !== undefined) {
				...
				if (discounts.length > 0) {
					...
					for (const discountDto of discounts) {
						...
						if (
							currentDiscountAmountUpdate.greaterThan(
								maxDiscountUpdate
							)
						) {
						...
						newFinalAmount = newFinalAmount.minus(
							currentDiscountAmountUpdate
						);
						totalDiscountAppliedOnUpdate =
							totalDiscountAppliedOnUpdate.plus(
								currentDiscountAmountUpdate
							);
						newOrderDiscountCreateInputs.push({
							discount_amount:
								currentDiscountAmountUpdate.toNumber(),
							discount: {
								connect: {
	...
	private async validateSingleDiscount(
		...
		let discountAmount = new Decimal(total_amount).times(
			discountPercentage
		);

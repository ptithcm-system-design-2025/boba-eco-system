...
interface CustomerState {
	...
	fetchCustomersByMembershipType: (
...
export const useCustomerStore = create<CustomerState>()(
	devtools(
		(set, get) => ({
			...
			fetchCustomersByMembershipType: async (
				membershipTypeId: number,
				params = {},
			) => {
				set({ isLoading: true, error: null });
				...
				try {
					...
					const response: PaginatedResponse<Customer> =
						await customerService.getByMembershipType(
							membershipTypeId,
							{
								page,
								limit,
					...
					const errorMessage = extractErrorMessage(
						error,
						'Không thể tải danh sách khách hàng theo loại thành viên',
					);
					set({
						error: errorMessage,
						isLoading: false,
			...
			bulkDeleteCustomers: async (data: BulkDeleteCustomerFormData) => {
				...
				try {
					...
					set((state) => ({
						customers: state.customers.filter(
							(customer) => !result.deleted.includes(customer.id),
						),
						selectedIds: new Set(),
						isDeleting: false,
					}));
					...
					if (result.summary.success > 0) {
						toast.success(
							`Đã xóa thành công ${result.summary.success} khách hàng`,
						);
					}

					if (result.summary.failed > 0) {
						toast.error(
							`Không thể xóa ${result.summary.failed} khách hàng`,
						);
					}

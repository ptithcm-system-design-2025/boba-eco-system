...
import type { CreateStoreDto, Store, UpdateStoreDto } from '@/types/store';
...
export const useStoreStore = create<StoreState>((set, get) => ({
	...
	fetchDefaultStore: async () => {
		try {
			...
			console.error('Lỗi fetch default store:', errorMessage);
			set({
				error: errorMessage,
				isLoading: false,
			});
	...
	fetchStores: async (page = 1, limit = 10) => {
		try {
			...
			console.error('Lỗi fetch stores:', errorMessage);
			set({
				error: errorMessage,
				isLoading: false,
			});
	...
	fetchStoreById: async (id: number) => {
		try {
			...
			console.error('Lỗi fetch store by id:', errorMessage);
	...
	createStore: async (data: CreateStoreDto) => {
		try {
			...
			console.error('Lỗi create store:', errorMessage);
			set({
				error: errorMessage,
				isCreating: false,
			});
	...
	updateStore: async (id: number, data: UpdateStoreDto) => {
		try {
			...
			console.error('Lỗi update store:', errorMessage);
			set({
				error: errorMessage,
				isUpdating: false,
			});
	...
	deleteStore: async (id: number) => {
		try {
			...
			console.error('Lỗi delete store:', errorMessage);
			set({
				error: errorMessage,
				isDeleting: false,
			});

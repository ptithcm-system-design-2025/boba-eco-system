...
interface ProductSizeState {
	...
	updateProductSize: (
		id: number,
		formData: UpdateProductSizeFormData,
	) => Promise<void>;
	deleteProductSize: (id: number) => Promise<void>;
	bulkDeleteProductSizes: (
		formData: BulkDeleteProductSizeFormData,
		...
		(set, get) => ({
			...
			fetchProductSizes: async (params) => {
				try {
					...
					const errorMessage = extractErrorMessage(
						error,
						'Lỗi tải danh sách kích thước sản phẩm',
					);
					set({
						error: errorMessage,
						isLoading: false,
			...
			getProductSizeById: async (id) => {
				try {
					...
					const errorMessage = extractErrorMessage(
						error,
						'Lỗi tải thông tin kích thước sản phẩm',
					);
					set({
						error: errorMessage,
						isLoading: false,
			...
			createProductSize: async (formData) => {
				try {
					...
					const newProductSize =
						await productSizeService.create(formData);
			...
			updateProductSize: async (id, formData) => {
				try {
					...
					const updatedProductSize = await productSizeService.update(
						id,
						formData,
					);
					...
					set((state) => ({
						...
						currentProductSize:
							state.currentProductSize?.id === id
								? updatedProductSize
					...
					const errorMessage = extractErrorMessage(
						error,
						'Lỗi cập nhật kích thước sản phẩm',
					);
					set({
						error: errorMessage,
						isUpdating: false,
			...
			deleteProductSize: async (id) => {
				try {
					...
					set((state) => ({
						productSizes: state.productSizes.filter(
							(ps) => ps.id !== id,
						),
						total: state.total - 1,
						currentProductSize:
			...
			bulkDeleteProductSizes: async (formData) => {
				try {
					...
					const result =
						await productSizeService.bulkDelete(formData);
					...
					const errorMessage = extractErrorMessage(
						error,
						'Lỗi xóa nhiều kích thước sản phẩm',
					);
					set({
						error: errorMessage,
						isDeleting: false,

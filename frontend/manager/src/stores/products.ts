...
interface ProductState {
	...
	bulkDeleteProducts: (
		data: BulkDeleteProductFormData,
	) => Promise<BulkDeleteResponse>;

	// Actions - Product Prices
	fetchProductPrices: (productId: number) => Promise<ProductPrice[]>;
	createProductPrice: (
		data: CreateProductPriceFormData,
	) => Promise<ProductPrice | null>;
	updateProductPrice: (
		priceId: number,
		data: UpdateProductPriceFormData,
	) => Promise<ProductPrice | null>;
	...
	devtools(
		(set, get) => ({
			...
			fetchProductPrices: async (productId: number) => {
				try {
					const prices =
						await productService.getProductPrices(productId);
					return prices;
				} catch (error: unknown) {
					const errorMessage = extractErrorMessage(
						error,
						'Không thể tải danh sách giá sản phẩm',
					);
					toast.error(errorMessage);
					return [];
				}
			...
			createProductPrice: async (data: CreateProductPriceFormData) => {
				...
				try {
					const newPrice =
						await productService.createProductPrice(data);

					// Refresh current product if it matches
					const { currentProduct } = get();
					if (
						currentProduct &&
						currentProduct.id === data.product_id
					) {
			...
			updateProductPrice: async (
				...
				try {
					const updatedPrice =
						await productService.updateProductPrice(priceId, data);
			...
			bulkDeleteProductPrices: async (priceIds: number[]) => {
				...
				try {
					const result =
						await productService.bulkDeleteProductPrices(priceIds);
					...
					if (result.summary.success > 0) {
						toast.success(
							`Đã xóa thành công ${result.summary.success} giá sản phẩm`,
						);
					}

					if (result.summary.failed > 0) {
						toast.error(
							`Không thể xóa ${result.summary.failed} giá sản phẩm`,
						);
					}
					...
					const errorMessage = extractErrorMessage(
						error,
						'Không thể xóa các giá sản phẩm đã chọn',
					);
					set({
						error: errorMessage,
						isDeletingPrice: false,

...
interface DiscountState {
	...
	bulkDeleteDiscounts: (ids: number[]) => Promise<BulkDeleteResponse>;
		...
		(set, get) => ({
			...
			fetchDiscounts: async (params = {}) => {
				...
				try {
					...
					const response: PaginatedResponse<Discount> =
			...
			fetchDiscountByCouponCode: async (couponCode: string) => {
				try {
					const discount =
						await discountService.getByCouponCode(couponCode);
					return discount;
				} catch (error) {
					const errorMessage = extractErrorMessage(error);
					console.error(
						'Lỗi lấy thông tin khuyến mãi theo mã:',
						errorMessage,
					);
					throw error;
				}
			...
			createDiscount: async (data: CreateDiscountDto) => {
				try {
					...
					const errorMessage = extractErrorMessage(error);
					console.error('Lỗi tạo khuyến mãi:', errorMessage);
			...
			updateDiscount: async (id: number, data: UpdateDiscountDto) => {
				try {
					...
					const errorMessage = extractErrorMessage(error);
					console.error('Lỗi cập nhật khuyến mãi:', errorMessage);
			...
			deleteDiscount: async (id: number) => {
				try {
					...
					const errorMessage = extractErrorMessage(error);
					console.error('Lỗi xóa khuyến mãi:', errorMessage);
			...
			bulkDeleteDiscounts: async (ids: number[]) => {
				try {
					...
					const errorMessage = extractErrorMessage(error);
					console.error('Lỗi xóa nhiều khuyến mãi:', errorMessage);
					set({ isDeleting: false, error: errorMessage });
					throw error;
				}
			},

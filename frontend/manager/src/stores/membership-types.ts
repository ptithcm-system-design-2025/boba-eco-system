...
interface MembershipTypeState {
	...
	fetchMembershipTypeByType: (
		type: string,
		includeCustomers?: boolean,
	) => Promise<void>;
	createMembershipType: (
		data: CreateMembershipTypeFormData,
	) => Promise<MembershipType | null>;
	updateMembershipType: (
		id: number,
		data: UpdateMembershipTypeFormData,
	) => Promise<MembershipType | null>;
		...
		(set, get) => ({
			...
			fetchMembershipTypes: async (params = {}) => {
				...
				try {
					...
					const response: PaginatedResponse<MembershipType> =
					...
					const errorMessage = extractErrorMessage(
						error,
						'Không thể tải danh sách loại thành viên',
					);
					set({
						error: errorMessage,
						isLoading: false,
			...
			fetchMembershipTypeById: async (
				...
				try {
					const membershipType = await membershipTypeService.getById(
						id,
						includeCustomers,
					);
					set({
					...
					const errorMessage = extractErrorMessage(
						error,
						'Không thể tải thông tin loại thành viên',
					);
					set({
						error: errorMessage,
						isLoading: false,
			...
			fetchMembershipTypeByType: async (
				type: string,
				includeCustomers = false,
			) => {
				set({ isLoading: true, error: null });
				...
				try {
					const membershipType =
						await membershipTypeService.getByType(
							type,
							includeCustomers,
						);
					set({
			...
			createMembershipType: async (
				...
				try {
					const newMembershipType =
						await membershipTypeService.create(data);

					// Refresh membership types list
					const { currentPage, itemsPerPage } = get();
					await get().fetchMembershipTypes({
						page: currentPage,
						limit: itemsPerPage,
					});
			...
			updateMembershipType: async (
				...
				try {
					const updatedMembershipType =
						await membershipTypeService.update(id, data);

					// Update membership types in current list
					set((state) => ({
						membershipTypes: state.membershipTypes.map(
							(membershipType) =>
						...
						currentMembershipType:
							state.currentMembershipType?.id === id
								? updatedMembershipType
								: state.currentMembershipType,
						isUpdating: false,
					}));
					...
					const errorMessage = extractErrorMessage(
						error,
						'Không thể cập nhật loại thành viên',
					);
					set({
						error: errorMessage,
						isUpdating: false,
			...
			deleteMembershipType: async (id: number) => {
				...
				try {
					...
					set((state) => ({
						membershipTypes: state.membershipTypes.filter(
							(membershipType) => membershipType.id !== id,
						),
						selectedIds: new Set(

...
// Form schema that matches the component's UI structure
...
type EditManagerFormValues = z.infer<typeof editManagerFormSchema>;

interface EditManagerFormProps {
	onSubmit: (data: UpdateManagerFormData) => Promise<void>;
	isSubmitting?: boolean;
	defaultValues?: Partial<EditManagerFormValues>;
}
...
export function EditManagerForm({
	...
	const handleSubmit = async (data: EditManagerFormValues) => {
		try {
			// Transform form data to backend format
			const [firstName, ...lastNameParts] = data.name.split(' ');
			const lastName = lastNameParts.join(' ') || firstName;

			const backendData: UpdateManagerFormData = {
					...
					render={({ field }) => (
						<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
							...
							<div className="space-y-1 leading-none">
								...
								<FormDescription>
									Cho phép quản lý viên này đăng nhập và sử
									dụng hệ thống
								</FormDescription>
							</div>
						</FormItem>

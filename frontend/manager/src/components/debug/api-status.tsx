...
export function ApiStatus() {
	...
	const checkApiConnection = useCallback(async () => {
		setApiStatus({ status: 'checking', message: 'Đang kiểm tra...' });
		...
		try {
			...
			if (response.ok) {
				...
				setApiStatus({
					status: 'connected',
					message: `Kết nối thành công - ${data.message || 'API hoạt động bình thường'}`,
				});
			} else {
				setApiStatus({
					status: 'disconnected',
			...
			setApiStatus({
				status: 'disconnected',
				message: `Không thể kết nối: ${error instanceof Error ? error.message : 'Lỗi không xác định'}`,
			});
		}
	}, []);
		...
		<Card className="w-full max-w-md">
			...
			<CardContent className="space-y-4">
				...
				<div className="space-y-2">
					...
					>
						<RefreshCw
							className={`mr-2 h-4 w-4 ${apiStatus.status === 'checking' ? 'animate-spin' : ''}`}
						/>
						Kiểm tra lại
					</Button>
				</div>

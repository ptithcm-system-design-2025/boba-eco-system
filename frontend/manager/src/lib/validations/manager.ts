...
export const bulkDeleteManagerSchema = z.object({
	ids: z.array(z.number()).min(1, 'Phải chọn ít nhất một quản lý để xóa'),
});
...
export function transformCreateManagerFormData(
...
export function transformUpdateManagerFormData(
	formData: UpdateManagerFormData,
) {
	const result: Record<string, unknown> = {};

	if (formData.first_name !== undefined)
		result.first_name = formData.first_name;

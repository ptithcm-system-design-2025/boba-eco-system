...
export const createMembershipTypeSchema = z.object({
	type: z
		...
		.refine(
			(value) => value.trim() !== '',
			'Tên loại thành viên không được chỉ chứa khoảng trắng',
		),
...
export type CreateMembershipTypeFormData = z.infer<
	typeof createMembershipTypeSchema
>;
export type UpdateMembershipTypeFormData = z.infer<
	typeof updateMembershipTypeSchema
>;

// Transform functions để chuyển đổi dữ liệu giữa frontend và backend
export function transformCreateMembershipTypeFormData(
...
export function transformUpdateMembershipTypeFormData(
	...
	if (formData.discountValue !== undefined)
		result.discount_value = formData.discountValue;
	if (formData.requiredPoint !== undefined)
		result.required_point = formData.requiredPoint;
	if (formData.description !== undefined)
		result.description = formData.description;
	// Chuyển đổi datetime-local (YYYY-MM-DDTHH:mm) thành date (YYYY-MM-DD)
	if (formData.validUntil !== undefined)
		result.valid_until = formData.validUntil
			? formData.validUntil.split('T')[0]
			: undefined;

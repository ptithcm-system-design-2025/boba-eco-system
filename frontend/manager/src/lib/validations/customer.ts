...
export const createCustomerSchema = z.object({
	lastName: z.string().max(70, 'Họ không được quá 70 ký tự').optional(),

	firstName: z.string().max(70, 'Tên không được quá 70 ký tự').optional(),
...
export type BulkDeleteCustomerFormData = z.infer<
	typeof bulkDeleteCustomerSchema
>;

// Transform functions để chuyển đổi dữ liệu giữa frontend và backend
export function transformCreateCustomerFormData(
...
export function transformUpdateCustomerFormData(
	formData: UpdateCustomerFormData,
) {
	const result: Record<string, unknown> = {};

	if (formData.membershipTypeId !== undefined)
		result.membership_type_id = formData.membershipTypeId;
	if (formData.lastName !== undefined) result.last_name = formData.lastName;
	if (formData.firstName !== undefined)
		result.first_name = formData.firstName;
	if (formData.phone !== undefined) result.phone = formData.phone;
	if (formData.currentPoints !== undefined)
		result.current_points = formData.currentPoints;

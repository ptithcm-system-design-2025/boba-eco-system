...
export type BulkDeleteProductSizeFormData = z.infer<
	typeof bulkDeleteProductSizeSchema
>;

// Transform functions để convert form data sang DTO backend
export function transformCreateProductSizeFormData(
	formData: CreateProductSizeFormData,
) {
	return {
		name: formData.name,
		unit: formData.unit,
...
export function transformUpdateProductSizeFormData(

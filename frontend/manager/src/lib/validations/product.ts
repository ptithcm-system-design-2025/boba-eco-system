...
export const createProductPriceItemSchema = z
	.object({
		size_id: z
			.number()
	...
	.refine(
		...
		{
			message:
				'Phải chọn kích thước có sẵn HOẶC tạo kích thước mới, không được có cả hai',
		},
	);
...
export const bulkDeleteProductSchema = z.object({
	ids: z.array(z.number()).min(1, 'Phải chọn ít nhất một sản phẩm để xóa'),
});

// Types được suy ra từ schemas
export type CreateProductPriceItemFormData = z.infer<
	typeof createProductPriceItemSchema
>;
export type CreateProductFormData = z.infer<typeof createProductSchema>;
export type UpdateProductFormData = z.infer<typeof updateProductSchema>;
export type CreateProductPriceFormData = z.infer<
	typeof createProductPriceSchema
>;
export type UpdateProductPriceFormData = z.infer<
...
export function transformCreateProductFormData(
...
export function transformUpdateProductFormData(
	...
	if (formData.is_signature !== undefined)
		result.is_signature = formData.is_signature;
	if (formData.image_path !== undefined)
		result.image_path = formData.image_path;
	if (formData.category_id !== undefined)
		result.category_id = formData.category_id;

...
// Role interface for user accounts
...
export function isValidRole(obj: unknown): obj is Role {
	return (
		typeof obj === 'object' &&
		obj !== null &&
		'role_id' in obj &&
		'name' in obj &&
		typeof (obj as Record<string, unknown>).role_id === 'number' &&
		typeof (obj as Record<string, unknown>).name === 'string'
	);
}

// Transform unknown role data to Role or return undefined
export function transformRole(roleData: unknown): Role | undefined {
	...
	if (typeof data.role_id === 'number' && typeof data.name === 'string') {
		return {
...
export function transformManagerResponse(
	backendManager: BackendManagerResponse,
): Manager {
	return {
		...
		lastLogin: backendManager.account?.last_login
			? new Date(backendManager.account.last_login)
			: undefined,
		role: transformRole(backendManager.account?.role),
	};
}
...
export function transformEmployeeResponse(
	backendEmployee: BackendEmployeeResponse,
): Employee {
	return {
		...
		lastLogin: backendEmployee.account?.last_login
			? new Date(backendEmployee.account.last_login)
			: undefined,
		role: transformRole(backendEmployee.account?.role),
	};
}
...
export function transformCustomerResponse(
	backendCustomer: BackendCustomerResponse,
): Customer {
	return {
		...
		lastLogin: backendCustomer.account?.last_login
			? new Date(backendCustomer.account.last_login)
			: undefined,
		role: transformRole(backendCustomer.account?.role),
		membershipType: backendCustomer.membership_type
			? transformMembershipTypeResponse(backendCustomer.membership_type)
			: undefined,
	};
}
...
export function transformMembershipTypeResponse(
	backendMembershipType: BackendMembershipTypeResponse,
): MembershipType {
	return {
		...
		validUntil: backendMembershipType.valid_until
			? new Date(backendMembershipType.valid_until)
			: undefined,
		...
		customers: backendMembershipType.customers?.map(
...
export function transformDiscountResponse(
	backendDiscount: BackendDiscountResponse,
): Discount {
	return {
		...
		validFrom: backendDiscount.valid_from
			? new Date(backendDiscount.valid_from)
			: undefined,
...
export function transformCategoryResponse(
	...
	return {
		...
		products: backendCategory.product
			? backendCategory.product.map(transformProductResponse)
			: undefined,
	};
}
...
export function transformProductSize(
	backendProductSize: BackendProductSizeResponse,
): ProductSize {
	return {
		...
		productPrices: backendProductSize.product_price
			? backendProductSize.product_price.map(
...
export function transformProductPriceResponse(
	backendProductPrice: BackendProductPriceResponse,
): ProductPrice {
	return {
		...
		product: backendProductPrice.product
			? transformProductResponse(backendProductPrice.product)
			: undefined,
		productSize: backendProductPrice.product_size
			? transformProductSize(backendProductPrice.product_size)
			: undefined,
	};
}
...
export function transformProductResponse(
	...
	const productPrices = backendProduct.product_price
		? backendProduct.product_price.map(transformProductPriceResponse)
		: [];
	...
	return {
		...
		category: backendProduct.category
			? transformCategoryResponse(backendProduct.category)
			: undefined,

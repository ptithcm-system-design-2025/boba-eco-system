...
export default function DiscountDetailPage() {
	...
	const discountId = parseInt(params.id as string, 10);
	...
	useEffect(() => {
		if (currentDiscount) {
			setFormData({
				...
				valid_from: currentDiscount.validFrom
					? format(currentDiscount.validFrom, "yyyy-MM-dd'T'HH:mm")
					: undefined,
				valid_until: format(
	...
	const validate = (): boolean => {
		...
		if (
			...
			newErrors.discount_value =
				'Giá trị giảm là bắt buộc và phải từ 1-100%';
		}

		if (
			!formData.min_required_order_value ||
			formData.min_required_order_value < 0
		) {
			newErrors.min_required_order_value =
				'Đơn hàng tối thiểu là bắt buộc và phải >= 0';
		}
		...
		if (
			...
			newErrors.max_discount_amount =
				'Số tiền giảm tối đa là bắt buộc và phải > 0';
		}
	...
	const handleSave = async () => {
		...
		try {
			...
			Object.keys(updateData).forEach((key) => {
				const typedKey = key as keyof UpdateDiscountDto;
			...
			toast.error(
				'Không thể cập nhật chương trình khuyến mãi. Vui lòng thử lại.',
			);
		}
	};
	...
	const handleDelete = async () => {
		try {
			...
			toast.error(
				'Không thể xóa chương trình khuyến mãi. Vui lòng thử lại.',
			);
		}
	};

	const handleChange = (
		field: keyof UpdateDiscountDto,
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div className="space-y-2">
									...
									{isEditing ? (
										<Input
											...
											className={
												errors.min_required_order_value
													? 'border-destructive'
													: ''
											}
										...
										<p className="text-sm">
											{formatCurrency(
												currentDiscount.minRequiredOrderValue,
											)}
										</p>
									)}
									{errors.min_required_order_value && (
								...
								<div className="space-y-2">
									<Label htmlFor="min_required_product">
									...
									{isEditing ? (
										...
										<p className="text-sm">
											{currentDiscount.minRequiredProduct ||
												'Không giới hạn'}
										</p>
									)}
								</div>
								...
								<div className="space-y-2">
									...
									{isEditing ? (
										<ClearableInput
											...
											min="1"
											value={
												formData.max_uses_per_customer ||
												''
											}
											onChange={(e) => {
												const value =
										...
										<p className="text-sm">
											{currentDiscount.maxUsesPerCustomer ||
												'Không giới hạn'}
										</p>
									)}
								</div>
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{currentDiscount.name}</strong>? Hành động

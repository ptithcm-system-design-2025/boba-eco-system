...
import type { CreateDiscountDto, Discount } from '@/types/discount';
...
export default function PromotionsPage() {
	...
	const handleCreateDiscount = async (data: CreateDiscountDto) => {
		try {
			...
			toast.error(
				'Không thể tạo chương trình khuyến mãi. Vui lòng thử lại.',
			);
		}
	};
	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			if (result.summary.failed > 0) {
				toast.warning(
					`Xóa thành công ${result.summary.success} chương trình. ${result.summary.failed} chương trình không thể xóa.`,
				);
			} else {
				toast.success(
					`Xóa thành công ${result.summary.success} chương trình khuyến mãi!`,
				);
			}
		} catch (error) {
			console.error('Lỗi xóa nhiều khuyến mãi:', error);
			toast.error(
				'Không thể xóa các chương trình khuyến mãi đã chọn. Vui lòng thử lại.',
			);
		}
	};
	...
	const columns: ColumnDef<Discount>[] = [
		...
		},
		{
			...
			cell: ({ row }) => {
				const currentUses =
					(row.getValue('currentUses') as number) || 0;
				...
				return (
					<DropdownMenu>
						...
						<DropdownMenuContent align="end">
							<DropdownMenuItem
								onClick={() =>
									router.push(`/promotions/${discount.id}`)
								}
							>
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Quản lý các chương trình giảm giá và khuyến mãi của cửa
						hàng
					</p>
				</div>
			...
			>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Tạo mới chương trình giảm giá và khuyến mãi cho
							khách hàng
						</DialogDescription>
					</DialogHeader>
					<CreateDiscountForm
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{selectedDiscount?.name}</strong>? Hành động
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							Xác nhận xóa nhiều chương trình
						</AlertDialogTitle>
						<AlertDialogDescription>
							Bạn có chắc chắn muốn xóa {selectedDiscounts.length}{' '}
							chương trình khuyến mãi đã chọn? Hành động này không
							thể hoàn tác.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>

	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			if (result.summary.failed > 0) {
				toast.warning(
					`Xóa thành công ${result.summary.success} sản phẩm. ${result.summary.failed} sản phẩm không thể xóa.`,
				);
			} else {
				toast.success(
					`Xóa thành công ${result.summary.success} sản phẩm!`,
				);
			}
			...
			toast.error(
				'Không thể xóa các sản phẩm đã chọn. Vui lòng thử lại.',
			);
		} finally {
			setIsDeleting(false);
		}
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{selectedProduct?.name}</strong>? Hành động
							này không thể hoàn tác và sẽ xóa tất cả giá liên
							quan.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							{selectedProductsArray.length} sản phẩm đã chọn khỏi
							danh mục <strong>{category?.name}</strong>? Hành
							động này không thể hoàn tác và sẽ xóa tất cả giá
							liên quan.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>

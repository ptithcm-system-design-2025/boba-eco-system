	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			if (result.summary.failed > 0) {
				toast.warning(
					`Xóa thành công ${result.summary.success} danh mục. ${result.summary.failed} danh mục không thể xóa.`,
				);
			} else {
				toast.success(
					`Xóa thành công ${result.summary.success} danh mục!`,
				);
			}
		} catch (error) {
			console.error('Lỗi xóa nhiều danh mục:', error);
			toast.error(
				'Không thể xóa các danh mục đã chọn. Vui lòng thử lại.',
			);
		}
	};
		...
		<div className="space-y-6">
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{selectedCategory?.name}</strong>? Hành động
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							Xác nhận xóa nhiều danh mục
						</AlertDialogTitle>
						<AlertDialogDescription>
							Bạn có chắc chắn muốn xóa{' '}
							{selectedCategories.length} danh mục đã chọn? Hành

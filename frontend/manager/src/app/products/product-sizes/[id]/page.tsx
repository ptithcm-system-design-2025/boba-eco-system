...
import {
	type UpdateProductSizeFormData,
...
export default function ProductSizeDetailPage({
	...
	const productSizeId = parseInt(resolvedParams.id, 10);
	...
	const loadProductSizeDetails = useCallback(async () => {
		try {
			setError(null);
	...
	}, [productSizeId, getProductSizeById]);

	useEffect(() => {
		if (productSizeId) {
			loadProductSizeDetails();
		}
	}, [productSizeId, loadProductSizeDetails]);
	...
	const handleSaveProductSize = async (data: UpdateProductSizeFormData) => {
		try {
			...
			toast.error(
				'Không thể cập nhật kích thước sản phẩm. Vui lòng thử lại.',
			);
		}
	};
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							? Hành động này không thể hoàn tác và có thể ảnh
							hưởng đến các sản phẩm đang sử dụng kích thước này.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>

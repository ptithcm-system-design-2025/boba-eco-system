...
import type { Product, ProductPrice, ProductSize } from '@/types/product';
...
export default function ProductDetailPage() {
	...
	const loadProductSizes = useCallback(async () => {
		try {
	...
	useEffect(() => {
		...
		loadCategories();
		loadProductSizes();
	}, [loadCategories, loadProductDetails, loadProductSizes]);
		...
		<div className="space-y-6">
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Danh sách giá sản phẩm theo từng kích
										thước
									</CardDescription>
								</div>
			...
			>
				<DialogContent>
					...
					<Form {...addPriceForm}>
						<form
							onSubmit={addPriceForm.handleSubmit(
								handleCreatePrice,
							)}
							className="space-y-4"
						>
			...
			>
				<DialogContent>
					...
					<Form {...editPriceForm}>
						<form
							onSubmit={editPriceForm.handleSubmit(
								handleUpdatePrice,
							)}
							className="space-y-4"
						>
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Bạn có chắc chắn muốn xóa giá{' '}
							<strong>
								{formatCurrency(selectedPrice?.price || 0)}
							</strong>
							cho kích thước{' '}
							<strong>{selectedPrice?.productSize?.name}</strong>?

...
// Utility function for formatting currency (currently unused but may be needed for future features)
			...
			return (
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						<DropdownMenuItem
							onClick={() =>
								router.push(`/users/customers/${customer.id}`)
							}
						>
...
const createMembershipTypeColumns = (
	router: ReturnType<typeof useRouter>,
): ColumnDef<MembershipType>[] => [
	{
		accessorKey: 'type',
		header: ({ column }) => (
			<Button
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						<DropdownMenuItem
							onClick={() =>
								router.push(
									`/users/membership-types/${membershipType.id}`,
								)
							}
						>
							<Eye className="mr-2 h-4 w-4" />
...
export default function CustomersPage() {
	...
	const {
		...
		selectCustomer,
		deselectCustomer,
		selectAllCustomers: _selectAllCustomers,
	...
	const {
		membershipTypes,
		currentPage: _membershipPage,
		totalPages: _membershipTotalPages,
		totalItems: membershipTotal,
		fetchMembershipTypes,
		isLoading: _membershipTypesLoading,
			...
			<div className="space-y-6">
				...
				<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
					...
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								{
									membershipTypes.filter((mt) => mt.isActive)
										.length
								}{' '}
								đang hoạt động
							</p>
				...
				>
					<TabsList>
						...
						<TabsTrigger value="membership-types">
							Loại Thành Viên
						</TabsTrigger>
					</TabsList>
										...
										<AlertDialogContent>
											<AlertDialogHeader>
												...
												<AlertDialogDescription>
													...
													{selectedIds.size} khách
													hàng đã chọn? Hành động này
													không thể hoàn tác.
												</AlertDialogDescription>
											</AlertDialogHeader>
											<AlertDialogFooter>
					...
					<TabsContent value="membership-types" className="space-y-4">
						...
						<Card>
							<CardContent className="p-0">
								<DataTable
									columns={createMembershipTypeColumns(
										router,
									)}

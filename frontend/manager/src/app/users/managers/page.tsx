...
import type { CreateManagerFormData } from '@/lib/validations/manager';
	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			if (result.summary.failed > 0) {
				toast.warning(
					`Xóa thành công ${result.summary.success} quản lý. ${result.summary.failed} quản lý không thể xóa.`,
				);
			} else {
				toast.success(
					`Xóa thành công ${result.summary.success} quản lý!`,
				);
			}
		} catch (error) {
		...
		<div className="space-y-6">
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{selectedManager?.name}</strong>? Hành động
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							Xác nhận xóa nhiều quản lý
						</AlertDialogTitle>
						<AlertDialogDescription>
							Bạn có chắc chắn muốn xóa {selectedManagers.length}{' '}
							quản lý đã chọn? Hành động này không thể hoàn tác.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel>Hủy</AlertDialogCancel>

...
export default function ManagerDetailPage() {
	...
	const managerId = parseInt(params.id as string, 10);
	...
	const handleSaveInfo = async (data: ManagerInfoFormData) => {
		...
		try {
			...
			const updatedManagerResponse = await apiClient.patch<unknown>(
				`/managers/${manager.id}`,
				managerUpdateData,
			);

			// Transform response để cập nhật store
			const responseData = updatedManagerResponse as Record<
				string,
				unknown
			>;
			const updatedManager: Manager = {
				...manager,
				id: Number(responseData.manager_id || manager.id),
				firstName: String(responseData.first_name || manager.firstName),
				lastName: String(responseData.last_name || manager.lastName),
				name: `${responseData.last_name || manager.lastName} ${responseData.first_name || manager.firstName}`,
				email: String(responseData.email || manager.email),
				phone: String(responseData.phone || manager.phone),
				gender: (responseData.gender || manager.gender) as
					| 'MALE'
					| 'FEMALE'
					| null,
				updatedAt: new Date(
					String(responseData.updated_at || new Date().toISOString()),
				),
			};
	...
	const handleSaveAccount = async (data: AccountInfoFormData) => {
		...
		try {
			...
			if (accountUpdateData.username) {
				const updatedManager: Manager = {
					...manager,
					username: String(accountUpdateData.username || ''),
					updatedAt: new Date(),
				};
				setSelectedManager(updatedManager);
			}
			...
			<AuthGuard>
				<div className="flex flex-col items-center justify-center h-96 space-y-4">
					...
					<p className="text-muted-foreground">
						Quản lý với ID {managerId} không tồn tại.
					</p>
						...
						<CardContent className="space-y-4">
							...
							<div className="flex items-center space-x-2">
								...
								<span className="text-sm">
									{formatGender(manager.gender)}
								</span>
							</div>
							<div className="flex items-center space-x-2">
								<Calendar className="h-4 w-4 text-muted-foreground" />
								<span className="text-sm">
									Tham gia {formatDate(manager.createdAt)}
								</span>
							</div>
							{manager.lastLogin && (
								<div className="flex items-center space-x-2">
							...
							<CardHeader className="flex flex-row items-center justify-between">
								...
								{!isEditingInfo ? (
									...
									<div className="flex space-x-2">
										...
										<Button
											size="sm"
											onClick={managerInfoForm.handleSubmit(
												handleSaveInfo,
											)}
											disabled={isUpdatingInfo}
										>
							...
							<CardContent>
								{isEditingInfo ? (
									<Form {...managerInfoForm}>
										<form
											onSubmit={managerInfoForm.handleSubmit(
												handleSaveInfo,
											)}
											className="space-y-4"
										>
							...
							<CardHeader className="flex flex-row items-center justify-between">
								...
								{!isEditingAccount ? (
									...
									<div className="flex space-x-2">
										...
										<Button
											size="sm"
											onClick={accountInfoForm.handleSubmit(
												handleSaveAccount,
											)}
											disabled={isUpdatingAccount}
										>

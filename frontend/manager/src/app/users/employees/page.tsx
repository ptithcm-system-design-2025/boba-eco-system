...
import type { CreateEmployeeFormData } from '@/lib/validations/employee';
...
export default function EmployeesPage() {
	...
	const handleBulkDelete = async () => {
		...
		try {
			const result = await bulkDeleteEmployees({
			...
			if (result.summary.failed > 0) {
				toast.warning(
					`Xóa thành công ${result.summary.success} nhân viên. ${result.summary.failed} nhân viên không thể xóa.`,
				);
			} else {
				toast.success(
					`Xóa thành công ${result.summary.success} nhân viên!`,
				);
			}
		} catch (error) {
			console.error('Lỗi xóa nhiều nhân viên:', error);
			toast.error(
				'Không thể xóa các nhân viên đã chọn. Vui lòng thử lại.',
			);
		}
	};
							...
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-2">
									<span className="text-sm text-muted-foreground">
										Đã chọn {selectedEmployees.length} nhân
										viên
									</span>
									<Button
										variant="outline"
				...
				>
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								Bạn có chắc chắn muốn xóa nhân viên &ldquo;
								{selectedEmployee?.name}&rdquo;? Hành động này
								không thể hoàn tác.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>

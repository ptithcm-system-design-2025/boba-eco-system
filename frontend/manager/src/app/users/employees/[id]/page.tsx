	...
	const loadEmployeeOrders = useCallback(async () => {
		...
		try {
			...
			if (Array.isArray(response)) {
				...
				ordersData = response.data as unknown[];
			} else if (
				response &&
				typeof response === 'object' &&
				'orders' in response &&
				Array.isArray((response as Record<string, unknown>).orders)
			) {
				ordersData = (response as Record<string, unknown>)
					.orders as unknown[];
			}
			...
			const transformedOrders: Order[] = ordersData.map(
				(order: unknown) => {
					...
					const customer = orderObj.customer as
						| Record<string, unknown>
						| undefined;
					return {
						id: Number(orderObj.id || orderObj.order_id || 0),
						customerName: String(
						...
						customerId: orderObj.customerId || orderObj.customer_id,
						employeeId: orderObj.employeeId || orderObj.employee_id,
						totalAmount: Number(
							orderObj.totalAmount || orderObj.total_amount || 0,
						),
						finalAmount: Number(
							orderObj.finalAmount ||
								orderObj.final_amount ||
								orderObj.totalAmount ||
								orderObj.total_amount ||
								0,
						),
						status: (orderObj.status || 'PROCESSING') as
							| 'PROCESSING'
							| 'CANCELLED'
							| 'COMPLETED',
		...
		return (
			<AuthGuard>
				<div className="flex flex-col items-center justify-center h-96 space-y-4">
					...
					<p className="text-muted-foreground">
						Nhân viên với ID {employeeId} không tồn tại.
					</p>
									...
									<CardHeader className="flex flex-row items-center justify-between">
										...
										{!isEditingInfo ? (
											...
											<div className="flex space-x-2">
												...
												<Button
													size="sm"
													onClick={employeeInfoForm.handleSubmit(
														handleSaveInfo,
													)}
													disabled={isUpdatingInfo}
												>
									...
									<CardContent>
										{isEditingInfo ? (
											<Form {...employeeInfoForm}>
												<form
													onSubmit={employeeInfoForm.handleSubmit(
														handleSaveInfo,
													)}
													className="space-y-4"
												>
									...
									<CardContent>
										{isLoadingOrders ? (
											...
											<div className="text-center py-8 text-muted-foreground">
												Nhân viên này chưa bán đơn hàng
												nào
											</div>
										) : (
											<Table>
